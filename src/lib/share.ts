...
export async function shareCard(blob: Blob): Promise<{ fallback: boolean }> {
  const file = new File([blob], "cooked-results.png", { type: "image/png" });

  if (canNativeShareFiles()) {
    try {
      await navigator.share({
        title: "My Cooked Results",
        text: "how cooked are you? getcooked.app",
        files: [file],
      });
...
export function downloadImage(blob: Blob): void {
  ...
  link.href = url;
  link.download = "cooked-results.png";
